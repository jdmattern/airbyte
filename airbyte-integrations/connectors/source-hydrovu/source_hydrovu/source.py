...
# abstractmethod may be needed at a later time
...
class SourceHydrovu(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        '''
           The streams method defines what our source can output
        '''
        # create session
        client = BackendApplicationClient(client_id=config["account_id"])
        oauth = OAuth2Session(client=client)
        ...
        # We might need below if specifying a start_date for the time series data
        #args = {"authenticator": authenticator, "account_id": config["account_id"], "start_date": start_date}

        return [
            #Pokemon(pokemon_name=config["pokemon_name"]),
            #GetListOfLocations(pokemon_name=config["pokemon_name"]),
            #GetListOfLocations(**args),
            #GetListOfLocations(pokemon_name=config["pokemon_name"], **args),
            
            #GetListOfLocations(**args),
            GetListOfLocations(oauth),
        ]


#class GetListOfLocations(HttpStream, ABC):
class GetListOfLocations(HttpStream):
